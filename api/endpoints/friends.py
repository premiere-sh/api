...
def get_friends(user_id: int, db: Session = Depends(get_db)):
    ...
    friend_usernames = [
        return_friend(friendship, user) for friendship in db_friendships
    ]
...
@router.post(
    "/users/{inviting_user_id}/friends/invite/{user_id}/",
    response_model=schemas.Friendship,
)
def send_invite(
    ...
    accepting_user = db.query(models.User).filter(models.User.id == user_id).first()
    ...
    db_friendship = (
        ...
        .filter(
            ...
            and models.Friendship.accepting_friend == accepting_user.username
        )
        .first()
    )
    if db_friendship:
...
@router.get("/users/{user_id}/invites/", response_model=List[schemas.Friendship])
def get_invites(
    user_id: int,
    ...
    db_friendships = (
        db.query(models.Friendship)
        .filter(models.Friendship.accepting_friend == current_user_db.username)
        .all()
    )
...
def accept_invite(
    ...
    if friendship.accepting_friend != current_user_db.username:
        raise HTTPException(status_code=403, detail="User unauthorized")
    db_query = db.query(models.Friendship).filter(
        (
            models.Friendship.inviting_friend == friendship.inviting_friend
            and models.Friendship.accepting_friend == user.username
        )
        and not models.Friendship.has_been_accepted
    )
...
def accept_invite(
    ...
    db_query = db.query(models.Friendship).filter(
        (
            models.Friendship.inviting_friend == friendship.inviting_friend
            and models.Friendship.accepting_friend == current_user_db.username
        )
        and not models.Friendship.has_been_accepted
    )
    db_friendship = db_query.first()
...
def unfriend(
    ...
    db_query = db.query(models.Friendship).filter(
        (
            models.Friendship.inviting_friend == user.username
            or models.Friendship.accepting_friend == user.username
        )
