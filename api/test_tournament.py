...
credentials = {"username": "user2", "password": "secret"}
...
def test_create_tournament():
    ...
    response = client.post("/tournaments/", json=sample_tournament, headers=headers)
...
def test_update_tournament_users():
    ...
    was_updated = (
        client.put("/tournaments/1", json=sample_tournament_with_users)
    ).json()
    assert was_updated
